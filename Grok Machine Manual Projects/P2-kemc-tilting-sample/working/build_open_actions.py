#!/usr/bin/env python3
"""Build Open Actions Register xlsx from machine_data.json (Grok sandbox only)."""
...
OUT = ROOT / "outputs" / f"KEMC_TiltingMachine_OpenActions_GrokSample_{DATA.get('revision', 'V1.0')}.xlsx"
...
def main():
    ...
    ws2["B3"] = DATA["status"]
    ws2["A4"] = "Revision"
    ws2["B4"] = DATA["revision"]
    ws2["A5"] = "Note"
    ws2["B5"] = "Standalone Open Actions Register — not embedded in OSM"
    for row in ws2.iter_rows(min_row=1, max_row=5, max_col=2):
        for cell in row:
